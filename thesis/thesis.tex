...
In 2008, the breakthrough result by \cite{DBLP:journals/corr/abs-0808-4134}, they gave an algorithm to solve the Symetric Diagonally Dominant(SDD) system of equations in 
near-linear time. This work was immediately extended by \cite{DBLP:journals/corr/abs-1003-2958} which gave an efficient algorithm to produce an incremental graph sparsifier.
...
% 	      \BEGIN \\ %
	  ...
	  \section{Dinic's Algorithm}
	   ...
	   \section{Push-Relabel Algorithms}
		...
		\item
	      ...
	      \textbf{Note}: pointer(u) represents the position of the pointer in the list currently. And neighbourList(u) represents list that 
	      maintains the vertices which neighbour u. And head(list) represents the first element of the list. \\
		...
		\Begin(Relabel-To-Front {(s,t)} :){
	      ...
	      \end{algorithm}
	      The procedure compute flow recursively calls the discharge function on different set of vertices and reorganizes the list. More formally,
	      it looks as follows \\
	      ...
	      \begin{prop}
	       The above Relabel-To-Front runs in time O($n^3$)
	      \end{prop}
	      
	      \begin{proof}
		We give a rough idea of the proof. For complete details, the reader is encouraged to refer \cite{clrs}.\\
		
		Define ``phase'' as number of operations between two calls to relabel function. Number of relabel operations is O($n^2$). 
		...
		and hence prove overall running time is O($n^3$).
	 ...
	 \chapter{Spectral Graphs}
	     ...
	     will be the key to understanding the breakthrough Cristiano-Kelner-Madry-Spielman-Teng algorithm and our attempts and observations. We only give the required amount of definitions and would avoid most proofs. This section is adapted from the monograph \cite{DBLP:journals/fttcs/Vishnoi13}.
	  ...
	  will be smaller owing to the upper bound on the average.
	   ...
	   \end{proof}
		...
		\section{Future Directions and Important Open Problems}
		 ...
		 get a parallel version of the same algorithm. This way it can be run in even larger graphs. It is also worthwhile to note that 
